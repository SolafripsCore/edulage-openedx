"""
Institution onboarding — a prospective institution asks to join from the marketplace site
(``POST /campus/api/v1/partner-requests/``); a marketplace administrator reviews it at
``/campus/admin/partners/``. Approval provisions the institution in one step: the Open edX
...
from campus_platform import identity, keycloak, provisioning
from campus_platform.console import _login_redirect, _page_context
...
ADMIN_PATH = "/campus/admin/partners/"
suggest_code = provisioning.suggest_code
code_available = provisioning.code_available


def record_request(data):
    """Store a request from the marketplace site and notify both sides; returns (request, created)."""
    ...
    emails.send_notice(
        [req.contact_email],
        f"We received your request to bring your institution onto {settings.PLATFORM_NAME}",
        "Partnership request", f"Thank you, {req.contact_name.split()[0]}",
        [
            f"We have received your request to bring {req.institution_name} onto {settings.PLATFORM_NAME} and will review it shortly.",
    ...
    if settings.MARKETPLACE_PARTNERS_EMAIL:
        emails.send_notice(
            [settings.MARKETPLACE_PARTNERS_EMAIL],
            f"New institution request: {req.institution_name}",
            "For review", f"{req.institution_name} wants to join {settings.PLATFORM_NAME}",
...
def provision_institution(code, name, admin_user, request_obj=None):
    """Approve a marketplace request: provision the tenant and invite the contact as first admin."""
    with transaction.atomic():
        _, invitation = provisioning.provision_institution(
            code, name, actor=admin_user,
            admin_email=request_obj.contact_email if request_obj else None,
            meta={"website": request_obj.website if request_obj else "", "country": request_obj.country if request_obj else ""},
        )
        if request_obj:
            ...
            request_obj.save()
        identity.audit("partner_approved", user=admin_user, email=request_obj.contact_email if request_obj else "", detail=f"{code}: {name}", actor="partners")
    return invitation
...
def _guard(request):
    ...
    if not (request.user.is_superuser or request.user.is_staff):
        return HttpResponseForbidden("Platform administrators only")
    return None
...
def admin_view(request):
    ...
    return render(request, "campus_marketplace/partners.html", context)
...
def approve_view(request, pk):
    ...
    back = redirect("campus_marketplace:partners")
    if not name:
    ...
    accept_url = request.build_absolute_uri(f"/campus/invite/{invitation.token}/")
    sent = emails.send_notice(
        [req.contact_email],
        f"{name} is approved on {settings.PLATFORM_NAME} — set up your institution",
        "Approved", f"Welcome to {settings.PLATFORM_NAME}, {name}",
        [
            f"{req.contact_name.split()[0]}, your request has been approved. {name} now exists on {settings.PLATFORM_NAME} with the institution code {code}.",
            f"Accept the invitation below with a {settings.PLATFORM_NAME} account registered to {req.contact_email} (sign in or create one) to become "
        ...
        [("Institution", name), ("Code", code), ("Institution console", f"https://{settings.LMS_BASE}/campus/institution/{code}/"),
         ("Studio", settings.CAMPUS_STUDIO_URL)],
...
def decline_view(request, pk):
    ...
    emails.send_notice(
        [req.contact_email],
        f"Your {settings.PLATFORM_NAME} partnership request",
        "Partnership request", f"About {req.institution_name} on {settings.PLATFORM_NAME}",
        [
            f"Thank you for your interest in bringing {req.institution_name} onto {settings.PLATFORM_NAME}. We are unable to proceed with the request at this time."
            + (f" Note from our team: {reason}" if reason else ""),
            "You are welcome to get in touch if circumstances change.",
        ],
        footnote=f"Questions? Write to {settings.MARKETPLACE_PARTNERS_EMAIL or settings.CONTACT_EMAIL}.",
    )
    messages.success(request, f"Request from {req.institution_name} declined.")
    return redirect("campus_marketplace:partners")
