"""
Institution console — ``/campus/institution/``: where an institution administrator sees who
holds staff roles at their institution, invites tutors/authors/administrators by e-mail, and
revokes access. Platform administrators see every institution.
...
``/campus/invite/<token>/``. Acceptance appends the claim to the account's ``campus_roles``
on the identity provider (which also switches on mandatory MFA through the ``campus-staff``
...
CONSOLE_PATH = "/campus/institution/"
...
def _page_context(request, **extra):
    return {
        ...
        "studio_url": f"{settings.CAMPUS_STUDIO_URL.rstrip('/')}/home" if settings.CAMPUS_STUDIO_URL else "",
...
def administered_institutions(user):
    """Org short names the user administers: all for platform admins, else institution_admin grants."""
...
def console_view(request, org=None):
    ...
    if not institutions:
        return render(request, "campus_platform/console_denied.html", _page_context(request), status=403)
    if org is None:
        return redirect("campus:console-org", org=institutions[0])
    ...
    return render(request, "campus_platform/console.html", context)
...
def invite_view(request, org):
    ...
    back = redirect("campus:console-org", org=org)
    ...
    if emails.send_staff_invitation(invitation, _org_name(org), request.build_absolute_uri(f"/campus/invite/{invitation.token}/")):
...
def _claim_in_org(claim, org):
    ...
    if name in identity.ORG_ROLES or name == "platform_support":
        return scope == org
...
def revoke_member_view(request, org, username):
    ...
    if username == request.user.username:
        messages.error(request, "You cannot remove your own access; ask another administrator or platform support.")
        return back
    ...
    try:
        ...
        log.warning("campus: revoke for %s at %s failed: %s", username, org, exc)
...
def invite_accept_view(request, token):
    ...
    if invitation.state != "pending":
        return render(request, "campus_platform/invite.html", {**context, "outcome": invitation.state}, status=410)
    ...
    if not request.user.is_authenticated:
        ...
        if request.GET.get("go") == "signin":
            return _login_redirect(request)
        return render(request, "campus_platform/invite.html", {**context, "outcome": "anonymous"})

    if (request.user.email or "").lower() != invitation.email.lower():
        return render(request, "campus_platform/invite.html", {**context, "outcome": "wrong-account", "account_email": request.user.email}, status=403)

    if request.method != "POST":
        return render(request, "campus_platform/invite.html", {**context, "outcome": "confirm"})
    ...
    try:
        ...
        log.warning("campus: accepting invitation %s failed: %s", invitation.pk, exc)
        return render(request, "campus_platform/invite.html", {**context, "outcome": "failed"}, status=503)
    ...
    return render(request, "campus_platform/invite.html", {**context, "outcome": "accepted"})
