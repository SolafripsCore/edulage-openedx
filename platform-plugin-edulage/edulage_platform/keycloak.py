"""
Thin client for the EduLage identity provider's admin API (Keycloak), used by the institution
console to grant and revoke staff roles.

Roles live on the IdP as the multi-valued user attribute ``edulage_roles`` (issued as the
``edulage_roles`` claim at sign-in and mirrored onto Open edX by ``identity.apply_roles``); the
``edulage-staff`` group carries the mandatory-MFA rule. Writing here rather than only to the LMS
keeps a single source of truth: the next sign-in reproduces exactly what the console set.

Authenticates as a confidential service-account client (``EDULAGE_KC_CLIENT_ID`` /
``EDULAGE_KC_CLIENT_SECRET``) that holds the realm-management roles ``view-users``,
...
ROLES_ATTRIBUTE = "edulage_roles"
STAFF_GROUP = "edulage-staff"
TIMEOUT = 10
...
def configured():
    return bool(settings.EDULAGE_KC_URL and settings.EDULAGE_KC_CLIENT_ID and settings.EDULAGE_KC_CLIENT_SECRET)


def _base():
    return settings.EDULAGE_KC_URL.rstrip("/")


def _token():
    try:
        r = requests.post(
            f"{_base()}/realms/{settings.EDULAGE_KC_REALM}/protocol/openid-connect/token",
            data={
                "grant_type": "client_credentials",
                "client_id": settings.EDULAGE_KC_CLIENT_ID,
                "client_secret": settings.EDULAGE_KC_CLIENT_SECRET,
            },
            timeout=TIMEOUT,
        )
        r.raise_for_status()
...
def _call(method, path, body=None, params=None):
    ...
    url = f"{_base()}/admin/realms/{settings.EDULAGE_KC_REALM}{path}"
    try:
        r = requests.request(
    ...
    if r.status_code >= 400:
        log.error("edulage: keycloak %s %s -> %s %s", method, path, r.status_code, r.text[:200])
        raise KeycloakError(f"identity provider returned {r.status_code}")
    return r.json() if r.content else {}


def get_user(sub):
    """Keycloak user representation for an EduLage account id, or None."""
...
def set_roles(kc_user, roles):
    """Replace the account's ``edulage_roles`` and align ``edulage-staff`` membership (MFA)."""
