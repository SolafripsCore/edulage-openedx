"""
Admissions wiring — the learner side (``/edulage/apply/<course_id>/``) and the institution side
(admissions panel in the institution console, plus CSV decision upload).

A learner applies from the programme page on edulage.org; this records an ``Admission`` in
...
from .models import Admission, CourseListing
...
def new_application_id():
    return f"EL-A-{secrets.token_hex(5).upper()}"
...
def apply_view(request, course_id):
    ...
    if listing is not None and listing.is_open:
    ...
    with transaction.atomic():
        ...
        if adm is None:
            adm = Admission.objects.create(
                user=request.user,
                edulage_sub=identity.sub_for_user(request.user),
    ...
    if created:
        ...
        emails.send_notice(
            ...
            [
                f"{institution} will review your application and record its decision on EduLage. "
        ...
        log.info("edulage: application %s %s → %s", adm.application_id, request.user.username, course_key)
...
def upload_view(request, org):
    ...
    for n, raw in enumerate(rows, start=2):
        ...
        if adm is None:
            ...
            adm = Admission.objects.create(
                user=user,
                edulage_sub=identity.sub_for_user(user),
                course_key=course_key,
                application_id=new_application_id(),
                institution=org,
                status=Admission.STATUS_SUBMITTED,
