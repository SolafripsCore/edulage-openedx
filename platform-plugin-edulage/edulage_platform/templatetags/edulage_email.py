"""Template helpers for the EduLage e-mail frame (``templates/overrides/ace_common``)."""
from django import template
from django.conf import settings

from ..emails import SITE_URL, brand_image_url
...
@register.simple_tag
def edulage_site_url(path=""):
    return f"{SITE_URL}/{path.lstrip('/')}" if path else SITE_URL


@register.simple_tag
def edulage_brand_image(name):
