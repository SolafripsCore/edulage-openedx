...
EduLage user id (OIDC ``sub``) to an Open edX user, projecting EduLage role claims onto Open
...
Role claim format (compact; issued in the ID token or pushed through the roles API):
    ...
    oec_support:<ORG>                   -> SupportScope(ORG)           (scoped, read-only; no Open edX role)
...
are synchronised by EduLage through ``PUT /edulage/api/v1/roles/`` (source ``api``) so ID
...
STAFF_CLAIMS = set(ORG_ROLES) | set(COURSE_ROLES) | {"edulage_admin", "oec_support"}


def audit(event, user=None, sub="", email="", detail="", actor="sso"):
    IdentityAudit.objects.create(user=user, edulage_sub=sub or "", email=email or "", event=event, detail=detail, actor=actor)


def user_for_sub(sub):
    """Open edX user linked to this EduLage identity, or None."""
    from social_django.models import UserSocialAuth  # pylint: disable=import-outside-toplevel

    link = UserSocialAuth.objects.filter(provider=EDULAGE_BACKEND, uid=sub).select_related("user").first()
...
def sub_for_user(user):
    ...
    link = UserSocialAuth.objects.filter(provider=EDULAGE_BACKEND, user=user).first()
    ...
    for claim in claims:
        ...
        if name in ORG_ROLES and scope:
            ...
            if scope:
                ...
                log.warning("edulage: ignoring unscoped oec_support claim (institution required)")
...
def revoke_all_roles(user, actor):
    """Remove every EduLage-managed grant (both sources) and Django staff flag."""
    from common.djangoapps.student.models import CourseAccessRole  # pylint: disable=import-outside-toplevel
    ...
    if user.is_staff and not user.is_superuser:
        user.is_staff = False
        user.save(update_fields=["is_staff"])
    log.info("edulage: revoked all managed roles for %s (%s)", user.username, actor)
...
def apply_pending_admissions(user, sub):
    ...
    pending = list(Admission.objects.filter(edulage_sub=sub, user__isnull=True))
    for adm in pending:
    ...
    Admission.objects.filter(user=user, edulage_sub__isnull=True).update(edulage_sub=sub)
    return len(pending)
...
def set_account_status(user, active, actor, reason=""):
    ...
    (Studio, MFEs, API clients), and every EduLage-managed role revoked. Course enrolments are
