...
Settings → Advanced settings → "Other course settings":
    ...
    {"edulage": {"enrolment_policy": "open_paid", "price": 25000, "currency": "NGN"}}
...
def parse_edulage_settings(other_course_settings):
    """Return ``{"enrolment_policy", "price", "currency"}`` or ``None`` when nothing is set / it is invalid."""
    raw = (other_course_settings or {}).get("edulage")
    ...
    if policy not in POLICIES:
        log.warning("edulage: ignoring unknown enrolment_policy %r", policy)
        return None
    try:
        price = Decimal(str(raw.get("price", 0))).quantize(Decimal("0.01"))
    except (InvalidOperation, ValueError):
        log.warning("edulage: ignoring invalid price %r", raw.get("price"))
        return None
    if policy == CourseListing.POLICY_OPEN_PAID and price <= 0:
        log.warning("edulage: open_paid run without a positive price; treating as admission")
        return None
...
def sync_listing_from_course(course_key):
    ...
    if course is None:
        return None
    values = parse_edulage_settings(course.other_course_settings)
    if values is None:
        return None
    ...
    log.info("edulage: %s enrolment_policy=%s price=%s %s", course_key, listing.enrolment_policy, listing.price, listing.currency)
    return listing
...
def connect_cms_signals():
    ...
    @receiver(SignalHandler.course_published, dispatch_uid="edulage_listing_sync")
    def _on_publish(sender, course_key, **kwargs):  # pylint: disable=unused-argument
        try:
            sync_listing_from_course(course_key)
        except Exception:  # pylint: disable=broad-except
            log.exception("edulage: listing sync failed for %s", course_key)
